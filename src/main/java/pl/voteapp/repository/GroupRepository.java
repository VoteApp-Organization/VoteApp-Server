...
public interface GroupRepository extends JpaRepository<Group__c, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM Group__c WHERE is_public = true AND LOWER(NAME) LIKE LOWER(CONCAT('%',:name,'%')) AND Id NOT IN :idList")
    public List<Group__c> exploreGroupByName(@Param("name") String name, @Param("idList") Set<Long> idList);
